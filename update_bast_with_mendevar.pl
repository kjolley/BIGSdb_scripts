...
sub main {
	...
	eval {
		$script->{'db'}->do( 'DELETE FROM profile_fields WHERE scheme_id=? AND scheme_field=?',
			undef, $opts{'scheme'}, 'Bexsero_reactivity' );
		$script->{'db'}->do( 'DELETE FROM profile_fields WHERE scheme_id=? AND scheme_field=?',
			undef, $opts{'scheme'}, 'Trumenba_reactivity' );
		foreach my $profile (@$bast_profiles) {
			...
			$script->{'db'}->do(
				'INSERT INTO profile_fields (scheme_id,scheme_field,profile_id,value,curator,datestamp) '
				  . 'VALUES (?,?,?,?,?,?)', undef, $opts{'scheme'}, 'Bexsero_reactivity', $profile->{'bast'}, $bexsero,
				0, 'now'
			);
			$script->{'db'}->do(
				'INSERT INTO profile_fields (scheme_id,scheme_field,profile_id,value,curator,datestamp) '
				  . 'VALUES (?,?,?,?,?,?)', undef, $opts{'scheme'}, 'Trumenba_reactivity', $profile->{'bast'}, $trumenba,
				0, 'now'
			);
